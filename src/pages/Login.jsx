...
export default function Login() {
  ...
  const handle = async (e) => {
    ...
    try {
      ...
      actions.setUser(auth.user)
    } catch (err) {
      setError(err?.message || 'Something went wrong. Check your credentials.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 380, textAlign: 'center' }}>
        ...
        <div style={{
          ...
          fontSize: 26, color: '#fff', fontWeight: 900, fontFamily: 'Georgia,serif',
          margin: '0 auto 20px', boxShadow: '0 8px 32px rgba(124,111,255,0.45)',
        }}>F</div>

        <h1 style={{ fontSize: 26, fontWeight: 900, color: 'var(--text)', letterSpacing: '-0.6px', marginBottom: 6 }}>
          Welcome to Finzo
        </h1>
        <p style={{ fontSize: 13, color: 'var(--sub)', marginBottom: 8 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, justifyContent: 'center', marginBottom: 28 }}>
          {['💸 Spending', '📈 Investments', '🏦 Net Worth', '🤖 AI Advisor'].map(f => (
            <span key={f} style={{
              ...
              borderRadius: 20, padding: '3px 10px', fontSize: 11, color: 'var(--sub)',
            }}>{f}</span>
          ))}
        </div>
        ...
        <div style={{
          ...
          <form onSubmit={handle}>
            {mode === 'signup' && (
              <div style={{ marginBottom: 12 }}>
                <label style={{ display: 'block', fontSize: 11, color: 'var(--sub)', fontWeight: 600, marginBottom: 5 }}>
                  Full Name
                </label>
                <input
                  className="input"
            ...
            <button type="submit" disabled={loading} style={{
              ...
              background: loading ? 'var(--muted)' : 'var(--gradient)',
          ...
          <p style={{ fontSize: 11, color: 'var(--muted)', marginTop: 12, lineHeight: 1.7 }}>
            🔒 Your data is stored privately. PDF parsing happens in your browser — no statement ever leaves your device.
          </p>
        </div>
      </div>
    </div>
